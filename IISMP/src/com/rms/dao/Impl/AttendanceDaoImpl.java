...
public class AttendanceDaoImpl extends BaseDao implements  AttendanceDao{
	...
	public List<Attendance_unusual> findmyappealmethod(int state, int m_id) {
		String sql = "";
		if(state!=0){
			sql = "from Attendance_unusual where applyuser_id="+m_id+" and state="+state+" order by timeinfo desc";
		} else{
			sql = "from Attendance_unusual where applyuser_id="+m_id+" order by timeinfo desc";
		}
		@SuppressWarnings("unchecked")
	...
	public List<Attendance_unusual> findAllpendingappeal() {
		@SuppressWarnings("unchecked")
		List<Attendance_unusual> au = hibernateTemplate.find("from Attendance_unusual order by timeinfo desc");
		return au;
	}
