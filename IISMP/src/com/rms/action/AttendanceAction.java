...
System.out.println("instate=="+instate);
	...
	public void checkinAction() throws Exception {
		...
		String datetime = DateUtil.getNowStrDate();
		//获取当前时间    HH:mm
		//String checkintime = DateUtil.getTimeHHmm();
...
System.out.println("DateUtil.getPreTimehhmm(check_out,tanxing_time)==="+DateUtil.getNextTimehhmm(check_out,tanxing_time));
	...
	public void savecheckrecordunusualinfo() throws Exception {
		...
		if(ad!=null && ad.size() >0){
			...
			a.setA_check_in_time(check_in);
			a.setA_check_out_time(check_out);
			a.setA_state("正常");
			a.setTime_difference(DateUtil.getTwoHour(check_out,check_in));
			//更新保存表Attendance
			attendanceService.updateAttendance(a);
		}else{
			Attendance a3 = new Attendance();
			a3.setA_check_in_time(check_in);
			a3.setA_check_out_time(check_out);
			a3.setA_state("正常");
			a3.setTime_difference(DateUtil.getTwoHour(check_out,check_in));
			a3.setA_datetime(aa.getDatetime());
			a3.setA_yueritime(DateUtil.getmmyueddri(aa.getDatetime()));
			a3.setM_id(m_id);
			a3.setA_nianyuetime(DateUtil.getNowDatenianyue());
			//保存到Attendance
...
System.out.println("a_id==="+a_id);
	...
	public String findcheckstatistics() throws Exception {
		...
		String nianyuetime = DateUtil.getYear()+"年"+DateUtil.getMonth()+"月";
		//计算当月有几天不用考勤
		Nokaoqin = DateUtil.getweeknum(nianyuetime, "星期六");
System.out.println("Nokaoqin=="+Nokaoqin);
		//计算当月到今天有几天
			...
			for(int i=0;i<msi.size();i++){
				...
				if(at1 != null && at1.size() > 0){
					for(int j=0;j<at1.size();j++){
						...
						if(a1.getA_state().equals("迟到")){
							allchidaon++;
							chidaon++;
						}else if(a1.getA_state().equals("早退")){
							allzaotuin++;
							zaotuin++;
						}else if(a1.getA_state().equals("未签退")){
							allweiqiantuin++;
							weiqiantuin++;
						}else if(a1.getA_state().equals("迟到，早退")){
							allchituin++;
							chituin++;
						}else if(a1.getA_state().equals("迟到，未签退")){
							allchiweituin++;
							chiweituin++;
						}
					}
...
System.out.println("allqueqinn***===="+allqueqinn);
	...
	public boolean exportexcelmethod(){
		...
		try {
			...
			if(tongji){
				...
				// 生成表头
				String[] bt = new String[] { "序号", "姓名", "分组", "应出勤/天", "实际出勤/天", "正常/天", "迟到/天", "早退/天", "未签退/天", "迟到，早退/天", "迟到，未签退/天", "缺勤/天"};
			...
			if(jilu){
				...
				// 生成表头
				String[] bt = new String[] { "序号", "分组", "姓名", "日期", "默认班次", "星期", "签到时间", "签退时间", "状态", "学习时长"};
				HSSFRow tableRow = sheet1.createRow(2); // 创建一个行对象
				tableRow.setHeightInPoints(25);
				for (int i = 0; i < bt.length; i++) {
					cell1 = tableRow.createCell(i);
					...
					if(at1 != null && at1.size() > 0){
						for(int j=0;j<at1.size();j++){
							...
							if(a1.getA_state().equals("迟到")){
								...
								chiweituin++;
							}else if(a1.getA_state().equals("正常")){
								zhengchang++;
							}
						}
						//计算time1到time2的缺勤数
