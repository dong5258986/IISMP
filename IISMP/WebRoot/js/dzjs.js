	...
	if(confirm("是否确定删除这条数据？")){
		$.ajax({
			...
			success:function(data) {
				...
				var thisstr="";
				$.each(str, function() {
					thisstr = thisstr + "<tr><td>" + this.as_name + "</td><td>" + this.as_updatetime + "</td><td>" + this.as_amount + "</td><td><a id='bianji01' href='#modal-container-184701' onclick='changeSectionByid("+this.as_id+")'  data-toggle='modal' style='text-decoration: none;'>编辑</a>&nbsp;&nbsp;<a href='javascript:void(0);' onclick='deleteAsection(" + this.as_id + ")' style='text-decoration: none'>删除</a></td></tr>";
				});
				document.getElementById('thistbody').innerHTML=thisstr;
			},
			error:function(data){
 ...
 */
function last(){
	var url1=url+'/findLast';
	var id1= $("#announcement_id11").html();
	...
	$.ajax({
			...
			success:function(data) {
				...
				alert("操作成功");
			  	var str = eval("("+data+")");
			  	document.getElementById("announcement_title").innerText=str.announcement_title;
	        	document.getElementById("views").innerHTML=str.announcement_views;
	        	document.getElementById("an_section").innerHTML=str.as_name;
	        	document.getElementById("an_time").innerHTML=str.announcement_time;
	        	document.getElementById("an_man").innerHTML=str.an_username;
	        	document.getElementById("announcement_info").innerHTML=str.announcement_info;
	        	document.getElementById("announcement_id11").innerHTML=str.announcement_id;
...
function findanno1(obj){
	...
	$.ajax({
		...
		success:function(data) {
			...
			$("#weifabunum").html(str.weifabunum);
			$("#yifabunum").html(str.yifabunum);
			$("#spinnerid").css('display','none'); 
		},
		error:function(data) {
			alert("操作失败，请刷新后重新操作！！");
