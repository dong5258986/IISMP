...
function opennewannocement() {
	var url0 = url + '/Functionality/announcement/anno_new.jsp';
...
function qiandaomethod() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (str.isCheckin) {
				if (str.instate) {
					checkin = "<h3 style='color: red'>"
							+ str.check_in_time + "</h3>";
				} else {
					checkin = "<h3 style='color: #999'>"
							+ str.check_in_time + "</h3>";
				}
			...
			if (str.isCheckout) {
				if (str.outstate) {
					checkout = "<h3 style='color: red'>"
							+ str.check_out_time + "</h3>";
				} else {
					checkout = "<h3 style='color: #999'>"
							+ str.check_out_time + "</h3>";
				}
...
function finduseratteninfo() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			var str = eval("(" + data + ")");
			if (str.isempty) {
				var thisstr = "<span >" + str.yueritime
						+ "</span><br><span>" + str.checkintime + "~";
 ...
 * 根据条件查询考勤统计
 */
function findthisstatsticsbyinfo() {
	spinnerstart();
	...
	$.ajax({
		...
		success : function(data) {
			...
			var sectionstr = '<option onclick="findthisstatsticsbyinfo();">全部</option>';
			$.each(str.jsonsection,function() {
				sectionstr = sectionstr
						+ '<option onclick="findthisstatsticsbyinfo();">'
						+ this.section_name
						+ '</option>';
			});
			$("#section_id").html(sectionstr);
